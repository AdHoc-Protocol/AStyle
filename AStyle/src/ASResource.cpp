...
const std::string ASResource::AS_INIT = std::string("init");
...
const std::string ASResource::AS_RECORD = std::string("record");
...
const std::string ASResource::AS_REPEAT = std::string("repeat");
const std::string ASResource::AS_GUARD = std::string("guard");
const std::string ASResource::AS_TRAIT = std::string("trait");
const std::string ASResource::AS_IMPL = std::string("impl");
const std::string ASResource::AS_RUST_MOD = std::string("mod");
const std::string ASResource::AS_OBJECT = std::string("object");
const std::string ASResource::AS_PROTOCOL = std::string("protocol");
...
const std::string ASResource::AS_NOT_EQUAL_JS = std::string("!==");
const std::string ASResource::AS_POWER = std::string("**");
const std::string ASResource::AS_POWER_ASSIGN = std::string("**=");
const std::string ASResource::AS_OR_OR_ASSIGN = std::string("||=");
const std::string ASResource::AS_AND_AND_ASSIGN = std::string("&&=");
const std::string ASResource::AS_QUESTION_DOT = std::string("?.");          // optional chaining
const std::string ASResource::AS_ELLIPSIS = std::string("...");             // spread, rest, and Swift closed range
const std::string ASResource::AS_RANGE = std::string("..");                 // Rust, Kotlin range, Dart cascade
const std::string ASResource::AS_RANGE_INCLUSIVE = std::string("..=");      // Rust inclusive range
const std::string ASResource::AS_RANGE_EXCLUSIVE = std::string("..<");      // Kotlin, Swift half open range
const std::string ASResource::AS_ELVIS = std::string("?:");                 // Kotlin elvis operator
const std::string ASResource::AS_NOT_NOT = std::string("!!");               // Kotlin not-null assertion
const std::string ASResource::AS_COLON_ASSIGN = std::string(":=");          // Go short variable declaration
const std::string ASResource::AS_CHANNEL = std::string("<-");               // Go channel operator
const std::string ASResource::AS_AND_NOT = std::string("&^");               // Go bit clear
const std::string ASResource::AS_AND_NOT_ASSIGN = std::string("&^=");
const std::string ASResource::AS_INT_DIVIDE = std::string("~/");            // Dart integer division
const std::string ASResource::AS_INT_DIVIDE_ASSIGN = std::string("~/=");
const std::string ASResource::AS_NULL_CASCADE = std::string("?..");         // Dart null aware cascade
const std::string ASResource::AS_NULL_SPREAD = std::string("...?");         // Dart null aware spread
 ...
 */
void ASResource::buildAssignmentOperators(std::vector<const std::string*>* assignmentOperators, int fileType)
{
	...
	if (fileType == JS_TYPE || fileType == TS_TYPE)
	{
		assignmentOperators->emplace_back(&AS_POWER_ASSIGN);
		assignmentOperators->emplace_back(&AS_OR_OR_ASSIGN);
		assignmentOperators->emplace_back(&AS_AND_AND_ASSIGN);
	}
	if (fileType == JS_TYPE || fileType == TS_TYPE || fileType == SWIFT_TYPE || fileType == DART_TYPE)
		assignmentOperators->emplace_back(&AS_COALESCE_CS);
	if (fileType == GO_TYPE)
	{
		assignmentOperators->emplace_back(&AS_COLON_ASSIGN);
		assignmentOperators->emplace_back(&AS_AND_NOT_ASSIGN);
	}
	if (fileType == DART_TYPE)
		assignmentOperators->emplace_back(&AS_INT_DIVIDE_ASSIGN);
...
void ASResource::buildHeaders(std::vector<const std::string*>* headers, int fileType, bool beautifier)
{
	...
	if (fileType >= GO_TYPE && fileType <= SCALA_TYPE)
	{
		...
		if (fileType == KOTLIN_TYPE || fileType == SWIFT_TYPE || fileType == DART_TYPE)
		{
...
void ASResource::buildNonParenHeaders(std::vector<const std::string*>* nonParenHeaders, int fileType, bool beautifier)
{
	...
	if (fileType == JAVA_TYPE || fileType == JS_TYPE || fileType == TS_TYPE)
	{
		nonParenHeaders->emplace_back(&AS_FINALLY);
	}
	...
	if (fileType == SHARP_TYPE)
	{
		...
		nonParenHeaders->emplace_back(&AS_INIT);
...
void ASResource::buildOperators(std::vector<const std::string*>* operators, int fileType)
{
	...
	if (fileType == JS_TYPE || fileType == TS_TYPE || fileType == KOTLIN_TYPE || fileType == SWIFT_TYPE)
	{
		operators->emplace_back(&AS_EQUAL_JS);
		operators->emplace_back(&AS_NOT_EQUAL_JS);
	}
	if (fileType == JS_TYPE || fileType == TS_TYPE)
	{
		operators->emplace_back(&AS_POWER);
		operators->emplace_back(&AS_POWER_ASSIGN);
		operators->emplace_back(&AS_OR_OR_ASSIGN);
		operators->emplace_back(&AS_AND_AND_ASSIGN);
	}
	if (fileType == SHARP_TYPE || fileType == JS_TYPE || fileType == TS_TYPE
	        || fileType == SWIFT_TYPE || fileType == DART_TYPE)
	{
		operators->emplace_back(&AS_COALESCE_CS);
	}
	if (fileType == JS_TYPE || fileType == TS_TYPE || fileType == KOTLIN_TYPE
	        || fileType == SWIFT_TYPE || fileType == DART_TYPE)
	{
		operators->emplace_back(&AS_QUESTION_DOT);
	}
	if (fileType == JS_TYPE || fileType == TS_TYPE || fileType == GO_TYPE
	        || fileType == SWIFT_TYPE || fileType == DART_TYPE)
	{
		operators->emplace_back(&AS_ELLIPSIS);
	}
	if (fileType == RUST_TYPE || fileType == KOTLIN_TYPE || fileType == DART_TYPE)
	{
		operators->emplace_back(&AS_RANGE);
	}
	if (fileType == RUST_TYPE)
	{
		operators->emplace_back(&AS_RANGE_INCLUSIVE);
	}
	if (fileType == KOTLIN_TYPE || fileType == SWIFT_TYPE)
	{
		operators->emplace_back(&AS_RANGE_EXCLUSIVE);
	}
	if (fileType == KOTLIN_TYPE)
	{
	...
	if (fileType == GO_TYPE)
	{
		operators->emplace_back(&AS_COLON_ASSIGN);
		operators->emplace_back(&AS_CHANNEL);
		operators->emplace_back(&AS_AND_NOT);
		operators->emplace_back(&AS_AND_NOT_ASSIGN);
	}
	if (fileType == DART_TYPE)
	{
		operators->emplace_back(&AS_INT_DIVIDE);
		operators->emplace_back(&AS_INT_DIVIDE_ASSIGN);
		operators->emplace_back(&AS_NULL_CASCADE);
		operators->emplace_back(&AS_NULL_SPREAD);
	}
...
void ASResource::buildPreBlockStatements(std::vector<const std::string*>* preBlockStatements, int fileType)
{
	...
	// Go and Rust have no classes
	if (fileType != GO_TYPE && fileType != RUST_TYPE)
	...
	if (fileType == SHARP_TYPE)
	{
		...
		preBlockStatements->emplace_back(&AS_STRUCT);
		preBlockStatements->emplace_back(&AS_RECORD);
	}
	if (fileType == TS_TYPE)
	{
		preBlockStatements->emplace_back(&AS_INTERFACE);
	...
	if (fileType == RUST_TYPE)
	{
		...
		preBlockStatements->emplace_back(&AS_IMPL);
		preBlockStatements->emplace_back(&AS_RUST_MOD);
	}
	if (fileType == KOTLIN_TYPE)
	{
		preBlockStatements->emplace_back(&AS_INTERFACE);
	...
	{
		preBlockStatements->emplace_back(&AS_STRUCT);
		preBlockStatements->emplace_back(&AS_ENUM);
		preBlockStatements->emplace_back(&AS_PROTOCOL);
		preBlockStatements->emplace_back(&AS_EXTENSION);
		preBlockStatements->emplace_back(&AS_ACTOR);
	}
	if (fileType == DART_TYPE)
	{
		preBlockStatements->emplace_back(&AS_MIXIN);
		preBlockStatements->emplace_back(&AS_EXTENSION);
	}
...
void ASResource::buildPreDefinitionHeaders(std::vector<const std::string*>* preDefinitionHeaders, int fileType)
{
	...
	if (fileType == SHARP_TYPE)
	{
		...
		preDefinitionHeaders->emplace_back(&AS_INTERFACE);
		preDefinitionHeaders->emplace_back(&AS_NAMESPACE);
		preDefinitionHeaders->emplace_back(&AS_RECORD);
	}
	if (fileType == TS_TYPE)
	{
		preDefinitionHeaders->emplace_back(&AS_INTERFACE);
	...
	if (fileType == RUST_TYPE)
	{
		...
		preDefinitionHeaders->emplace_back(&AS_IMPL);
		preDefinitionHeaders->emplace_back(&AS_RUST_MOD);
	}
	if (fileType == KOTLIN_TYPE)
	{
		preDefinitionHeaders->emplace_back(&AS_INTERFACE);
	...
	{
		preDefinitionHeaders->emplace_back(&AS_STRUCT);
		preDefinitionHeaders->emplace_back(&AS_ENUM);
		preDefinitionHeaders->emplace_back(&AS_PROTOCOL);
		preDefinitionHeaders->emplace_back(&AS_EXTENSION);
		preDefinitionHeaders->emplace_back(&AS_ACTOR);
	}
	if (fileType == DART_TYPE)
	{
		preDefinitionHeaders->emplace_back(&AS_MIXIN);
		preDefinitionHeaders->emplace_back(&AS_EXTENSION);
	}
