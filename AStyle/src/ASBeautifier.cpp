...
ASBeautifier::ASBeautifier()
{
	...
	headers = std::make_shared<std::vector<const std::string*>>();
	nonParenHeaders = std::make_shared<std::vector<const std::string*>>();
	assignmentOperators = std::make_shared<std::vector<const std::string*>>();
	nonAssignmentOperators = std::make_shared<std::vector<const std::string*>>();
	preBlockStatements = std::make_shared<std::vector<const std::string*>>();
	preCommandHeaders = std::make_shared<std::vector<const std::string*>>();
	indentableHeaders = std::make_shared<std::vector<const std::string*>>();
...
ASBeautifier::ASBeautifier(const ASBeautifier& other) : ASBase(other)
{
	...
	// (default-constructed to nullptr, as before)

	// vector copy construction performs a DEEP copy of all elements in the vector

	headerStack = std::make_unique<std::vector<const std::string*>>(*other.headerStack);

	tempStacks = copyTempStacks(other);

	parenDepthStack = std::make_unique<std::vector<int>>(*other.parenDepthStack);

	blockStatementStack = std::make_unique<std::vector<bool>>(*other.blockStatementStack);

	parenStatementStack = std::make_unique<std::vector<bool>>(*other.parenStatementStack);

	braceBlockStateStack = std::make_unique<std::vector<bool>>(*other.braceBlockStateStack);

	continuationIndentStack = std::make_unique<std::vector<int>>(*other.continuationIndentStack);

	continuationIndentStackSizeStack = std::make_unique<std::vector<size_t>>(*other.continuationIndentStackSizeStack);

	parenIndentStack = std::make_unique<std::vector<int>>(*other.parenIndentStack);

	preprocIndentStack = std::make_unique<std::vector<std::pair<int, int> >>(*other.preprocIndentStack);

	lambdaDepthStack = std::make_unique<std::vector<int>>(*other.lambdaDepthStack);
	...
	lineBraceIsBlock = other.lineBraceIsBlock;
	lineBraceIndex = other.lineBraceIndex;
	savedContinuations = other.savedContinuations;
}

/**
 * ASBeautifier's destructor
 * All member vectors are smart pointers, so cleanup is automatic.
 */
ASBeautifier::~ASBeautifier() = default;
...
void ASBeautifier::init(ASSourceIterator* iter)
{
	...
	// block continuation is the default for JavaScript, TypeScript and the new languages
	if (blockContinuationMode < 0)
		blockContinuation = isJSStyle() || isNewLanguageStyle();
	else
		blockContinuation = (blockContinuationMode == 1);
	...
	waitingBeautifierStack = std::make_unique<std::vector<std::unique_ptr<ASBeautifier>>>();
	activeBeautifierStack = std::make_unique<std::vector<std::unique_ptr<ASBeautifier>>>();

	waitingBeautifierStackLengthStack = std::make_unique<std::vector<size_t>>();
	activeBeautifierStackLengthStack = std::make_unique<std::vector<size_t>>();

	headerStack = std::make_unique<std::vector<const std::string*>>();

	tempStacks = std::make_unique<std::vector<std::unique_ptr<std::vector<const std::string*>>>>();
	tempStacks->emplace_back(std::make_unique<std::vector<const std::string*>>());

	parenDepthStack = std::make_unique<std::vector<int>>();
	blockStatementStack = std::make_unique<std::vector<bool>>();
	parenStatementStack = std::make_unique<std::vector<bool>>();
	braceBlockStateStack = std::make_unique<std::vector<bool>>();
	// do not use emplace_back on std::vector<bool> until supported by macOS
	braceBlockStateStack->push_back(true);
	continuationIndentStack = std::make_unique<std::vector<int>>();
	continuationIndentStackSizeStack = std::make_unique<std::vector<size_t>>();
	continuationIndentStackSizeStack->emplace_back(0);
	parenIndentStack = std::make_unique<std::vector<int>>();
	preprocIndentStack = std::make_unique<std::vector<std::pair<int, int> >>();
	lambdaDepthStack = std::make_unique<std::vector<int>>();
...
void ASBeautifier::initVectors()
{
	...
	ASResource::buildHeaders(headers.get(), fileType, true);
	ASResource::buildNonParenHeaders(nonParenHeaders.get(), fileType, true);
	ASResource::buildAssignmentOperators(assignmentOperators.get(), fileType);
	ASResource::buildNonAssignmentOperators(nonAssignmentOperators.get(), fileType);
	ASResource::buildPreBlockStatements(preBlockStatements.get(), fileType);
	ASResource::buildPreCommandHeaders(preCommandHeaders.get(), fileType);
	ASResource::buildIndentableHeaders(indentableHeaders.get());
}
...
std::string ASBeautifier::beautify(const std::string& originalLine)
{
	...
	else if (isInComment || isInBeautifySQL)
	{
		...
		if (size_t firstChar = line.find_first_not_of(" \t"); firstChar != std::string::npos)
		{
			if (line[firstChar] == '{')
	            ...
	            || backslashEndsPrevLine))
	{
		if (line[0] == '#' && !isInDefine)
		{
			...
			if (shouldIndentPreprocConditional && !preproc.empty())
			{
				...
				if (preproc.length() >= 2 && preproc.substr(0, 2) == "if") // #if, #ifdef, #ifndef
				{
					// indentCount, spaceIndentCount
					std::pair<int, int> entry = (!isInDefine && activeBeautifierStack != nullptr && !activeBeautifierStack->empty())
					                            ? activeBeautifierStack->back()->computePreprocessorIndent()
		...
		if (!backslashEndsPrevLine && isInDefineDefinition && !isInDefine)
		{
			...
			if (activeBeautifierStack->empty() || isIndentModeOff || shouldPreserveIndent)
				return originalLine;
			std::unique_ptr<ASBeautifier> defineBeautifier = std::move(activeBeautifierStack->back());
			activeBeautifierStack->pop_back();
	...
	if (!isInDefine && activeBeautifierStack != nullptr && !activeBeautifierStack->empty())
	{
		...
		activeBeautifierStack->back()->lineBraceIsBlock = lineBraceIsBlock;
		activeBeautifierStack->back()->lineBraceIndex = 0;
	...
	// A line beginning with a member access or a binary operator continues the
	// statement of the previous line (e.g. a chain of method calls). It is
	// indented unless it is already in a continuation.
	// A line following a line ending with a binary operator continues the statement.
	bool isContinuedStatement = isMaskedStyle()
	                            && parenDepth == 0
	...
	if (isInRustWhereClause && (lineBeginsWithOpenBrace || lineIsCommentOnly || line.empty()))
		isInRustWhereClause = lineIsCommentOnly;
	if (isInRustWhereClause)
	...
	else if (blockContinuation && parenDepth > 0 && !lineStartsInComment
	...
	if (!lineIsCommentOnly && !lineIsLineCommentOnly && !lineStartsInComment)
	{
		...
		if (isRustStyle())
		{
			...
			if (last != std::string::npos && (line[last] == ';' || line[last] == '{'))
				isInRustWhereClause = false;
			else if (line == "where" || (line.length() > 6 && line.compare(line.length() - 6, 6, " where") == 0))
				isInRustWhereClause = true;
		}
	}
	...
	indentCount = std::max(0, indentCount);
...
/**
 * set indentation style to TypeScript.
 */
void ASBeautifier::setTSStyle()
{
	fileType = TS_TYPE;
}

/**
 * set indentation style to Go.
 */
void ASBeautifier::setGoStyle()
{
	fileType = GO_TYPE;
}

/**
 * set indentation style to Rust.
 */
void ASBeautifier::setRustStyle()
{
	fileType = RUST_TYPE;
}

/**
 * set indentation style to Kotlin.
 */
void ASBeautifier::setKotlinStyle()
{
	fileType = KOTLIN_TYPE;
}

/**
 * set indentation style to Swift.
 */
void ASBeautifier::setSwiftStyle()
{
	fileType = SWIFT_TYPE;
}

/**
 * set indentation style to Dart.
 */
void ASBeautifier::setDartStyle()
{
	fileType = DART_TYPE;
}
...
/**
 * set JSX mode. If true, JSX elements are recognized in JavaScript and TypeScript.
 */
void ASBeautifier::setJSXMode(bool state)
{
	jsxMode = state;
}

bool ASBeautifier::getJSXMode() const
{
	return jsxMode;
}
...
/**
 * set the continuation indentation mode.
 * In block mode continuation lines are indented from the start of the
 * statement instead of being aligned with an assignment, and an opening
 * paren or bracket ending a line opens an indented block.
 *
 * @param   mode    -1 = use the language default, 0 = align, 1 = block.
 */
void ASBeautifier::setBlockContinuationMode(int mode)
{
	blockContinuationMode = mode;
}

bool ASBeautifier::getBlockContinuation() const
{
                                              ...
                                              int tabIncrementIn, int minIndent, bool updateParenStack)
{
	...
	// Block continuation: an assignment or an opening paren or bracket that ends
	// the line is indented from the start of the line, and the closing paren or
	// bracket is aligned with the start of the line.
	// a literal brace followed only by other openers does not add an indent either,
	// e.g. the Go composite literal '[]T{{'
	if (blockContinuation && i >= 0 && line[i] == '{' && isFollowedOnlyByOpeners(line, i))
	{
		continuationIndentStack->emplace_back(spaceIndentCount_);
		if (updateParenStack)
			parenIndentStack->emplace_back(spaceIndentCount_);
		return;
	}
	if (blockContinuation && i >= 0 && line[i] != '{')
	{
		bool isOpener = (line[i] == '(' || line[i] == '[' || line[i] == '<');
		// an opener followed only by other openers does not add an indent,
		// the last opener on the line adds the indent, e.g. 'foo({' or '=> ({'
		if (isOpener && isFollowedOnlyByOpeners(line, i))
		{
			continuationIndentStack->emplace_back(spaceIndentCount_);
			if (updateParenStack)
				parenIndentStack->emplace_back(spaceIndentCount_);
		...
		// the continuation is indented from the start of the line, it is not aligned
		// with the paren, a header condition has at least the minimum conditional indent
		{
			...
			if (!isOpener && !continuationIndentStack->empty()
			        && continuationIndentStack->back() > currIndent)
				currIndent = continuationIndentStack->back();
...
void ASBeautifier::registerContinuationIndentColon(std::string_view line, int i, int tabIncrementIn)
{
	...
	if (size_t firstChar = line.find_first_not_of(" \t"); firstChar == (size_t) i)		// firstChar is ':'
	{
		if (size_t firstWord = line.find_first_not_of(" \t", firstChar + 1); firstWord != std::string::npos)
		{
 ...
 *
 * @return              the index number of element in the container. std::nullopt if element not found.
 * @param container     a vector of std::strings.
 * @param element       the element to find .
 */
std::optional<int> ASBeautifier::indexOf(const std::vector<const std::string*>& container, const std::string* element) const
{
 ...
 */
std::unique_ptr<std::vector<std::unique_ptr<std::vector<const std::string*>>>> ASBeautifier::copyTempStacks(const ASBeautifier& other) const
{
	auto tempStacksNew = std::make_unique<std::vector<std::unique_ptr<std::vector<const std::string*>>>>();
	for (const auto& stack : *other.tempStacks)
		tempStacksNew->emplace_back(std::make_unique<std::vector<const std::string*>>(*stack));
 ...
 */
bool ASBeautifier::isJSMemberName(std::string_view line, size_t i, const std::string* header, bool isDefinition) const
{
	if (!isJSStyle())
		return false;
	size_t next = line.find_first_not_of(" \t", i + header->length());
	if (next != std::string_view::npos
	        && ((line[next] == ':' && line.compare(next, 2, "::") != 0)
	            || line.compare(next, 2, "?:") == 0)
	        && header != &ASResource::AS_DEFAULT && header != &ASResource::AS_CASE)
		return true;
	// in an object literal
	if (parenDepth == 0 && !braceBlockStateStack->empty() && !braceBlockStateStack->back())
		return true;
	// in a class or an interface body, a definition may be nested
	if (!isDefinition && headerStack->size() >= 2
	        && headerStack->back() == &ASResource::AS_OPEN_BRACE
...
/**
 * Check if a 'switch' is a C# switch expression, e.g. "x switch { ... }".
 * A switch expression follows an expression and is followed by a brace,
 * which may be on the next line.
 */
bool ASBeautifier::isSharpSwitchExpression(std::string_view line, size_t i) const
{
	if (!isSharpStyle())
		return false;
	if (!(isLegalNameChar(prevNonSpaceCh) || prevNonSpaceCh == ')' || prevNonSpaceCh == ']' || prevNonSpaceCh == '"'))
		return false;
	// the previous char is the 's' of 'switch' when it begins the statement
	if (i == 0 || line.find_first_not_of(" \t") == i)
		return false;
	size_t next = line.find_first_not_of(" \t", i + ASResource::AS_SWITCH.length());
	return next == std::string_view::npos || line[next] == '{' || line.compare(next, 2, "//") == 0;
}

/**
 * Check if 'record' begins a C# record declaration.
 */
bool ASBeautifier::isSharpRecordHeader(std::string_view line, size_t i) const
{
	size_t next = line.find_first_not_of(" \t", i + ASResource::AS_RECORD.length());
	if (next == std::string_view::npos || next == i + ASResource::AS_RECORD.length())
		return false;
	if (!isCharPotentialHeader(line, next))
		return false;
	std::string_view word = getCurrentWord(line, next);
	return !(word == "is" || word == "as" || word == "in" || word == "and" || word == "or"
	         || word == "not" || word == "with" || word == "switch" || word == "when");
}

/**
 * Get the indent of the lines of the innermost block, it is not zero for a
 * block in a continuation, e.g. a lambda in the arguments of a call.
 */
int ASBeautifier::getBlockContinuationBase() const
{
	if (!savedContinuations.empty() && savedContinuations.back().isSaved)
		return savedContinuations.back().base;
	return 0;
}

/**
 * Check if a position in a line is in type arguments, e.g. "Iterator<Item = T>".
 * The angle brackets are counted from the start of the line.
 */
bool ASBeautifier::isInAngleBrackets(std::string_view line, size_t i) const
{
	int angles = 0;
	for (size_t j = 0; j < i && j < line.length(); j++)
	{
		char ch = line[j];
		char next = j + 1 < line.length() ? line[j + 1] : ' ';
		char prev = j > 0 ? line[j - 1] : ' ';
		if (ch == '<' && next != '=' && next != '<' && prev != '<')
			++angles;
		else if (ch == '>' && prev != '-' && prev != '=' && next != '=')
			--angles;
		else if (ch == '"')
		{
...
/**
 * Check if the code of a line ends with a binary operator, so the statement
 * continues on the next line, e.g. "a &&" or the Go "x.".
 */
bool ASBeautifier::endsWithOperator(std::string_view line) const
{
	// remove a line end comment, there are no quotes in the masked source
	size_t end = line.length();
	size_t comment = line.find("//");
	if (comment != std::string_view::npos)
		end = comment;
	comment = line.find("/*");
	if (comment != std::string_view::npos && comment < end)
		end = comment;
	size_t last = line.find_last_not_of(" \t", end == 0 ? 0 : end - 1);
	if (last == std::string_view::npos || end == 0)
		return false;
	char ch = line[last];
	char prev = last > 0 ? line[last - 1] : ' ';
	// an import of all names, e.g. "import kotlinx.coroutines.*"
	if (line.compare(0, 7, "import ") == 0)
		return false;
	...
	if (ch == '&' || ch == '|' || ch == '+' || ch == '-' || ch == '*' || ch == '/' || ch == '%' || ch == '^')
		return true;
	// a lambda arrow, but not an assignment which is a continuation already
	return ch == '>' && (prev == '=' || prev == '-');
}

/**
 * Get the paren a generic angle bracket acts as. An opening angle bracket that
 * directly follows a name and ends the line opens a generic list spanning lines,
 * it acts as '('. The matching closing angle bracket acts as ')'.
 * Other angle brackets are returned unchanged.
 */
char ASBeautifier::getAngleBlockChar(std::string_view line, size_t i, char ch)
{
	char prev = i > 0 ? line[i - 1] : ' ';
	char next = i + 1 < line.length() ? line[i + 1] : ' ';
	// not an arrow, a comparison or a shift
	if (ch == '>' && (prev == '-' || prev == '=' || next == '='))
		return ch;
	if (ch == '<' && (next == '=' || next == '<' || prev == '<'))
		return ch;
	if (ch == '<' && isLegalNameChar(prev) && getNextProgramCharDistance(line, i) == (int) (line.length() - i))
	{
		angleBlockDepths.push_back(1);
		return '(';
	}
	if (angleBlockDepths.empty())
		return ch;
	if (ch == '<')
	{
		++angleBlockDepths.back();
		return ch;
	}
	if (--angleBlockDepths.back() > 0)
		return ch;
	angleBlockDepths.pop_back();
	return ')';
}

/**
 * Check if the remainder of a line after an opening paren or bracket contains
 * only opening parens, brackets and braces, e.g. 'foo({' or '=> ({'.
 */
bool ASBeautifier::isFollowedOnlyByOpeners(std::string_view line, size_t i) const
{
...
/**
 * Check if a trimmed line begins with a token that continues the statement
 * of the previous line: a member access ('.', '?.', '..'), a binary or logical
 * operator, a null-coalescing operator, or the parts of a conditional operator.
 */
bool ASBeautifier::isLeadingContinuation(std::string_view line) const
{
	if (line.empty())
		return false;
	char first = line[0];
	...
	if (first == '.')
		return true;
	// a Kotlin property accessor, e.g. "val x: T" and "get() = y" on the next line
	if (isKotlinStyle()
	        && (line.compare(0, 4, "get(") == 0 || line.compare(0, 4, "set(") == 0
	            || line.compare(0, 5, "get =") == 0 || line.compare(0, 11, "private set") == 0
	            || line.compare(0, 12, "internal set") == 0 || line.compare(0, 13, "protected set") == 0))
		return true;
	if (first == '?')
		return true;
	// a binary operator, not a unary '&' address-of or a '|' closure
	// a Rust pattern alternative is not indented
	if ((first == '&' || first == '|') && (second == first || second == ' ' || second == '\t')
	        && !(isRustStyle() && first == '|' && second != '|'))
		return true;
...
void ASBeautifier::processPreprocessor(std::string_view preproc, std::string_view line)
{
	...
	if (shouldIndentPreprocDefine && preproc == "define" && line[line.length() - 1] == '\\')
	{
		if (!isInDefineDefinition)
		{
			...
			activeBeautifierStack->emplace_back(std::make_unique<ASBeautifier>(*this));
		}
		else
		{
	...
	else if (preproc.length() >= 2 && preproc.substr(0, 2) == "if")
	{
		...
		if (activeBeautifierStackLengthStack->back() == 0)
			waitingBeautifierStack->emplace_back(std::make_unique<ASBeautifier>(*this));
		else
			waitingBeautifierStack->emplace_back(std::make_unique<ASBeautifier>(*activeBeautifierStack->back()));
	}
	else if (preproc == "else")
	{
		if ((waitingBeautifierStack != nullptr) && !waitingBeautifierStack->empty())
		{
			...
			activeBeautifierStack->emplace_back(std::move(waitingBeautifierStack->back()));
	...
	else if (preproc == "elif")
	{
		if ((waitingBeautifierStack != nullptr) && !waitingBeautifierStack->empty())
		{
			...
			activeBeautifierStack->emplace_back(std::make_unique<ASBeautifier>(*waitingBeautifierStack->back()));
		}
	}
	else if (preproc == "endif")
	{
...
void ASBeautifier::computePreliminaryIndentation()
{
	...
	for (size_t i = 0; i < headerStack->size(); i++)
	{
		...
		// TypeScript namespaces are always indented
		if (!isJavaStyle() && !isJSStyle() && !namespaceIndent && i > 0
	...
	if (isInClassHeader)
	{
		...
		// the parameters of a primary constructor or a record are indented
		// by the paren only, not also by the class header
		// a Rust item header is not indented, e.g. a where clause
		if (isMaskedStyle() && (parenDepth > 0 || isRustStyle()) && indentCount > 0 && !lineBeginsWithOpenBrace)
			--indentCount;
...
int ASBeautifier::adjustIndentCountForBreakElseIfComments() const
{
	...
	std::vector<const std::string*>* lastTempStack = tempStacks->back().get();
...
void ASBeautifier::adjustObjCMethodDefinitionIndentation(std::string_view line_)
{
	        ...
	        && (line_[0] == '-' || line_[0] == '+'))
	{
		if (shouldAlignMethodColon && objCColonAlignSubsequent != -1)
		{
			...
			colonIndentObjCMethodAlignment = findObjCColonAlignment(convertedLine).value_or(-1);
...
void ASBeautifier::adjustObjCMethodCallIndentation(std::string_view line_)
{
	...
	if (shouldAlignMethodColon && objCColonAlignSubsequent != -1)
	{
		...
		{
			if (findObjCColonAlignment(line_).has_value())
			{
				if (colonIndentObjCMethodAlignment < 0)
 ...
 */
std::optional<int> ASBeautifier::findObjCColonAlignment(std::string_view line) const
{
...
{
	std::optional<int> colonPosition = findObjCColonAlignment(line);
	if (!colonPosition.has_value() || *colonPosition > colonAlignPosition)
		return indentLength;
...
{
	const std::string* newHeader = findHeader(line, *i, headers.get());

	// a keyword may be a property or a method name in JavaScript and TypeScript
	if (newHeader != nullptr && isJSMemberName(line, *i, newHeader))
		newHeader = nullptr;
	...
	if (newHeader == &ASResource::AS_CASE && isSwiftStyle() && headerStack->size() >= 2
	        && headerStack->back() == &ASResource::AS_OPEN_BRACE
	        && (*headerStack)[headerStack->size() - 2] == &ASResource::AS_ENUM)
		newHeader = nullptr;
	if (newHeader == &ASResource::AS_ELSE && isSwiftStyle() && prevNonSpaceCh != '}')
		newHeader = nullptr;

	// a C# switch expression is not a switch statement
	if (newHeader == &ASResource::AS_SWITCH && isSharpSwitchExpression(line, *i))
		newHeader = nullptr;

	// a C# init accessor follows an opening brace, a semicolon, or an accessor modifier
	if (newHeader == &ASResource::AS_INIT
	        && prevNonSpaceCh != '{' && prevNonSpaceCh != ';' && prevNonSpaceCh != '}' && prevNonSpaceCh != ']'
	...
	{
		size_t next = line.find_first_not_of(" \t", *i + newHeader->length());
		if (!isJavaStyle() || next == std::string_view::npos || line.compare(next, 2, "->") != 0)
			newHeader = nullptr;
	}
	...
	if (newHeader != nullptr)
	{
		...
		if (!tempStacks->empty())
			lastTempStack = tempStacks->back().get();
		...
		else if (newHeader == &ASResource::AS_ELSE && !lambdaDepth)
		{
			if (lastTempStack != nullptr)
			{
				std::optional<int> indexOfIf = indexOf(*lastTempStack, &ASResource::AS_IF);
				if (indexOfIf.has_value())
				{
		...
		else if (newHeader == &ASResource::AS_WHILE)
		{
			if (lastTempStack != nullptr)
			{
				std::optional<int> indexOfDo = indexOf(*lastTempStack, &ASResource::AS_DO);
				if (indexOfDo.has_value())
				{
		...
		else if (newHeader == &ASResource::AS_CATCH || newHeader == &ASResource::AS_FINALLY)
		{
			if (lastTempStack != nullptr)
			{
				std::optional<int> indexOfTry = indexOf(*lastTempStack, &ASResource::AS_TRY);
				if (!indexOfTry.has_value())
		...
		if (isIndentableHeader)
		{
			...
			isContinuation = false;
			if (!indexOf(*nonParenHeaders, newHeader).has_value())
			{
	...
	if (findHeader(line, *i, preCommandHeaders.get()) != nullptr)
...
bool ASBeautifier::handleColonSection(std::string_view line, size_t* i, bool tabIncrementIn, char* ch)
{
	...
	else
	{
		...
		// a Go label is unindented by one indent, as gofmt does
		else if (isGoStyle() && peekedChar == ' ' && *i > 0 && line.find_first_of(" \t") > *i)
		{
			--indentCount;
		}
...
void ASBeautifier::handleEndOfStatement(size_t i, bool *closingBraceReached, char* ch)
{
	...
	if (*ch == '}')
	{
		...
		if (!parenDepthStack->empty())
		{
			...
			// restore the continuation of the statement containing the block
			if (!savedContinuations.empty())
			{
				SavedContinuation& saved = savedContinuations.back();
				if (saved.isSaved)
				{
					*continuationIndentStack = std::move(saved.indents);
		...
		*closingBraceReached = true;
		if (i == 0)
			spaceIndentCount = closingBraceIndent;
		isInAsmBlock = false;
		...
		std::optional<int> headerPlace = indexOf(*headerStack, &ASResource::AS_OPEN_BRACE);
		if (headerPlace.has_value())
		{
			...
			if (!namespaceIndent && !isJSStyle() && !headerStack->empty()
...
void ASBeautifier::handleClosingParen(std::string_view line, size_t i, bool tabIncrementIn)
{
	...
	// Also exclude block-scope openers like 'namespace xxx {' or 'extern "C" {'
	// where the brace opens a scope rather than initializing a variable.
	// For extern "C", prevNonSpaceCh is not reliable here: it is not updated
	// while scanning the quoted "C" token, so it still holds a stale legal
	// identifier character left over from 'extern', which would otherwise
	// make isDirectBraceInit true below.
	...
	// use the brace type determined by ASFormatter, if available
	// except for JavaScript, an initializer brace ending a line is indented as a block
	bool isFormatterArrayBrace = false;
	if (lineBraceIndex < lineBraceIsBlock.size())
	{
		bool isFormatterBlock = lineBraceIsBlock[lineBraceIndex++];
		isFormatterArrayBrace = !isFormatterBlock;
		if (isFormatterBlock)
			isBlockOpener = true;
		else if (isJSStyle() || !isNonInStatementArray || isInClassInitializer || isInConditional)
			isBlockOpener = false;
		// a literal brace followed only by other openers adds no indent, e.g. '[]T{{'
		else if (blockContinuation && isFollowedOnlyByOpeners(line, i))
			isBlockOpener = false;
	}
	...
	// A block in a continuation, e.g. a lambda in the arguments of a call or
	// a method in an object literal, is indented from the line it begins on.
	// A block brace beginning a line is aligned with the previous line.
	// The continuation of the statement is restored by the closing brace.
	SavedContinuation saved;
	if (isMaskedStyle()
	        && (parenDepth > 0 || !continuationIndentStack->empty() || isContinuedStatementLine)
	        && !isInClassInitializer && !isInClassHeaderTab
	        && !(isFormatterArrayBrace && lineBeginsWithOpenBrace))
	{
		saved.isSaved = true;
		saved.indents = *continuationIndentStack;
		saved.sizes = *continuationIndentStackSizeStack;
		int base = spaceIndentCount;
		if (lineBeginsWithOpenBrace && line.find_first_not_of(" \t") == i)
		{
		...
		if (base > 0)
		{
			continuationIndentStack->emplace_back(base);
			continuationIndentStackSizeStack->emplace_back(1);
		}
		else
			continuationIndentStackSizeStack->emplace_back(0);
	...
	else if (!continuationIndentStack->empty())
	{
		...
		if (isMaskedStyle())
		{
			// purge the continuationIndentStack down to the base of a block in a continuation
			while (continuationIndentStackSizeStack->size() > 1)
				popLastContinuationIndent();
			size_t bottom = continuationIndentStackSizeStack->empty() ? 0 : continuationIndentStackSizeStack->back();
			while (continuationIndentStack->size() > bottom)
				continuationIndentStack->pop_back();
			if (!continuationIndentStack->empty())
				base = continuationIndentStack->back();
			saved.base = base;
		}
		else
		{
...
void ASBeautifier::handlePotentialHeaderSection(std::string_view line, size_t* i, bool tabIncrementIn, bool *isInOperator)
{
	...
	// a type definition is not in parens, e.g. the Rust "fn f(x: impl Trait)"
	if (!isInTemplate && !((isCStyle() || isJSStyle() || isNewLanguageStyle()) && parenDepth > 0))
	{
		const std::string* newHeader = findHeader(line, *i, preBlockStatements.get());
		// a keyword may be a property name in JavaScript and TypeScript
		if (newHeader != nullptr && isJSMemberName(line, *i, newHeader, true))
			newHeader = nullptr;
		// a C# record is a class, 'record' may also be an identifier
		if (newHeader == &ASResource::AS_RECORD)
		{
			if (isSharpRecordHeader(line, *i))
			{
				*i += newHeader->length() - 1;
				newHeader = &ASResource::AS_CLASS;
				if (!headerStack->empty() && (headerStack->back() == &ASResource::AS_CLASS
				                              || headerStack->back() == &ASResource::AS_STRUCT))
					return;
				headerStack->emplace_back(newHeader);
		                 ...
		                 || headerStack->back() != &ASResource::AS_OPEN_BRACE)))
		{
			...
			if (!headerStack->empty())
			{
				const std::string* definition = (*headerStack).back();
				if (definition == &ASResource::AS_CLASS
				        || definition == &ASResource::AS_STRUCT
				        || definition == &ASResource::AS_INTERFACE
				        || definition == &ASResource::AS_IMPL
				        || definition == &ASResource::AS_TRAIT
				        || definition == &ASResource::AS_RUST_MOD
				        || definition == &ASResource::AS_OBJECT
				        || definition == &ASResource::AS_PROTOCOL
				        || definition == &ASResource::AS_EXTENSION
				        || definition == &ASResource::AS_ACTOR
				        || definition == &ASResource::AS_MIXIN
				        || (definition == &ASResource::AS_ENUM && isSwiftStyle()))
				{
					isInClassHeader = true;
				}
	...
	const std::string* foundIndentableHeader = findHeader(line, *i, indentableHeaders.get());
...
void ASBeautifier::handlePotentialOperatorSection(std::string_view line, size_t* i, bool tabIncrementIn, bool* haveAssignmentThisLine, bool isInOperator)
{
	...
	const std::string* foundAssignmentOp = findOperator(line, *i, assignmentOperators.get());
	const std::string* foundNonAssignmentOp = findOperator(line, *i, nonAssignmentOperators.get());
	...
	// an '=' in type arguments is not an assignment, e.g. "Iterator<Item = T>"
	else if (foundAssignmentOp != nullptr && isMaskedStyle() && !isSharpStyle() && !isJavaStyle()
	         && foundAssignmentOp == &ASResource::AS_ASSIGN && isInAngleBrackets(line, *i))
	{
	}
	...
	for (size_t i = 0; i < line.length(); i++)
	{
		...
		if (std::isblank(ch))
		{
			...
			if (squeezeWhitespace && !isInComment && !isInQuote && !nextTokenIsComment && std::isblank(line[i + 1]) && !std::isblank(line[i - 1]))
				squeezeWSStack.emplace_back(i, wsSpanEnd - i - 1);
		...
		// a generic parameter or argument list spanning lines is indented like a paren,
		// e.g. "fn process<" ... ">(items: Vec<T>)"
		if (blockContinuation && isMaskedStyle() && (ch == '<' || ch == '>'))
		{
			ch = getAngleBlockChar(line, i, ch);
			currentNonSpaceCh = ch;
		}
		...
		// a Go ':=' is not a colon
		if (ch == ':' && !(isGoStyle() && line.compare(i, 2, ":=") == 0))
		{
			if (!handleColonSection(line, &i, tabIncrementIn, &ch))
				continue;
		}
		// the arrow of a Java case label ends the label like a colon, e.g. "case 1 -> {"
		else if (ch == '-' && isInCase && isJavaStyle() && parenDepth == 0
		         && line.compare(i, 2, "->") == 0)
		{
			++i;
			isInCase = false;
