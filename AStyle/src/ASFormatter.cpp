...
ASFormatter::ASFormatter()
{
    ...
    enhancer = std::make_unique<ASEnhancer>();
    ...
    headers = std::make_unique<std::vector<const std::string*>>();
    nonParenHeaders = std::make_unique<std::vector<const std::string*>>();
    preDefinitionHeaders = std::make_unique<std::vector<const std::string*>>();
    preCommandHeaders = std::make_unique<std::vector<const std::string*>>();
    operators = std::make_unique<std::vector<const std::string*>>();
    assignmentOperators = std::make_unique<std::vector<const std::string*>>();
    castOperators = std::make_unique<std::vector<const std::string*>>();

    // initialize ASEnhancer member std::vectors
    indentableMacros = std::make_unique<std::vector<const std::pair<const std::string, const std::string>* >>();
}

/**
 * Destructor of ASFormatter
 * All member vectors and the enhancer are smart pointers, so cleanup is automatic.
 */
ASFormatter::~ASFormatter() = default;
...
void ASFormatter::init(ASSourceIterator* si)
{
    ...
    enhancer->init(getFileType(),
                   ...
                   getEmptyLineFill(),
                   indentableMacros.get(),
                   shouldPreserveIndent);

    // the ASFormatter object is never deleted, so any existing vectors
    // must be replaced (not merely populated) before starting a new file
    preBraceHeaderStack = std::make_unique<std::vector<const std::string*>>();
    parenStack = std::make_unique<std::vector<int>>();
    structStack = std::make_unique<std::vector<bool>>();
    questionMarkStack = std::make_unique<std::vector<bool>>();
    parenStack->emplace_back(0);               // parenStack must contain this default entry
    braceTypeStack = std::make_unique<std::vector<BraceType>>();
    braceTypeStack->emplace_back(BraceType::NULL_TYPE);   // braceTypeStack must contain this default entry
    ...
    checksumIn = 0;
    checksumOut = 0;
    currentLineFirstBraceNum = std::nullopt;
    formattedLineCommentNum = 0;
    leadingSpaces = 0;
    previousReadyFormattedLineLength = std::nullopt;
    preprocBraceTypeStackSize = 0;
    spacePadNum = 0;
    methodAttachCharNum = std::nullopt;
    methodAttachLineNum = 0;
    methodBreakCharNum = std::nullopt;
    ...
    runInIndentChars = 0;
    tabIncrementIn = 0;
    previousBraceType = BraceType::NULL_TYPE;
    ...
    formattedLineBraceBlocks.clear();
    readyFormattedLineBraceBlocks.clear();
    isSharpSwitchExpressionPending = false;
    isLambdaSignatureLine = false;
    nonInStatementArrayDepths.clear();
    functionBodyParenDepth = -1;
    matchBodyParenDepth = -1;
}
...
void ASFormatter::buildLanguageVectors()
{
    ...
    ASResource::buildHeaders(headers.get(), formatterFileType);
    ASResource::buildNonParenHeaders(nonParenHeaders.get(), formatterFileType);
    ASResource::buildPreDefinitionHeaders(preDefinitionHeaders.get(), formatterFileType);
    ASResource::buildPreCommandHeaders(preCommandHeaders.get(), formatterFileType);
    ASResource::buildOperators(operators.get(), formatterFileType);
    ASResource::buildAssignmentOperators(assignmentOperators.get(), formatterFileType);
    ASResource::buildCastOperators(castOperators.get());
    ASResource::buildIndentableMacros(indentableMacros.get());	//ASEnhancer
}
...
bool ASFormatter::handleImmediatelyPostHeaderSection()
{
            ...
            && (shouldBreakOneLineStatements || !isHeaderInMultiStatementLine))
    {
        ...
        if (!nextText.empty()
                ...
                && ASBase::findHeader(nextText, 0, headers.get()) == &ASResource::AS_IF)
        {
            isInLineBreak = true;
        }
    }
...
bool ASFormatter::handlePassedSemicolonSection()
{
    ...
    if (parenStack->back() == 0 && !isCharImmediatelyPostComment && currentChar != ';') // allow ;;
    {
        ...
        if (isBraceType(braceTypeStack->back(), BraceType::SINGLE_LINE_TYPE))
        {
...
void ASFormatter::handleBraces()
{
    ...
    if (currentChar == '{' && !appendOpeningBrace)
    {
        ...
        isSharpSwitchExpressionPending = false;
        if (isLambdaSignatureFollows())
            isLambdaSignatureLine = true;
        if (functionBodyParenDepth == parenStack->back())
            functionBodyParenDepth = -1;
        if (matchBodyParenDepth == parenStack->back())
            matchBodyParenDepth = -1;
        breakCurrentOneLineBlock = false;
        foundNamespaceHeader = false;
        foundClassHeader = false;
        ...
        isPreviousBraceBlockRelated = !isBraceType(newBraceType, BraceType::ARRAY_TYPE);
        braceTypeStack->emplace_back(newBraceType);
        // a conditional operator does not continue into an initializer
        if (isMaskedStyle() && isBraceType(newBraceType, BraceType::ARRAY_TYPE))
        {
        ...
        if (isBraceType(newBraceType, BraceType::STRUCT_TYPE) && isCStyle())
    ...
    bool isOpeningArrayBrace = (isBraceType(braceType, BraceType::ARRAY_TYPE)
                                && braceTypeStack->size() >= 2
                                && !isBraceType((*braceTypeStack)[braceTypeStack->size() - 2], BraceType::ARRAY_TYPE)
                               );
    ...
    if (currentChar == '}')
    {
        ...
        if (braceTypeStack->size() > 1)
        {
            ...
            isPreviousBraceBlockRelated = !isBraceType(braceType, BraceType::ARRAY_TYPE);
        }
        else
        {
            previousBraceType = BraceType::NULL_TYPE;
        ...
        // restore the conditional operator of the statement containing the initializer
        if (isMaskedStyle() && isBraceType(braceType, BraceType::ARRAY_TYPE) && !questionMarkStack->empty())
        {
            foundQuestionMark = questionMarkStack->back();
            questionMarkStack->pop_back();
        }
        ...
        // a non in-statement array ends with its own closing brace, not with the
        // closing brace of a block in the array, e.g. a lambda
        if (isMaskedStyle())
        {
            if (!nonInStatementArrayDepths.empty() && nonInStatementArrayDepths.back() == braceTypeStack->size() + 1)
            {
                nonInStatementArrayDepths.pop_back();
                if (isNonInStatementArray && nonInStatementArrayDepths.empty())
                    isImmediatelyPostNonInStmt = true;
            }
        }
        else if (isNonInStatementArray
                && (!isBraceType(braceTypeStack->back(), BraceType::ARRAY_TYPE)	// check previous brace
    ...
    // format braces
    appendOpeningBrace = false;
    if (isBraceType(braceType, BraceType::ARRAY_TYPE))
    {
    ...
    // record the brace type for ASBeautifier
    // the brace is not appended if it is moved to the following line
    if (currentChar == '{' && isMaskedStyle())
        formattedLineBraceBlocks.push_back(!isBraceType(braceType, BraceType::ARRAY_TYPE));
}

/**
 * Get the brace type of an opening brace in JavaScript and TypeScript.
 * A brace is a block if it is the body of a function, a method, an arrow
 * function, a class or a control statement, otherwise it is an object literal.
 */
BraceType ASFormatter::getJSBraceType() const
{
    // an object type in type arguments, e.g. Record<string, { a: T }>
    if (isInTemplate)
        return BraceType::ARRAY_TYPE;
    // class, interface and namespace bodies, not a brace in the parens of a declaration
    if (foundPreDefinitionHeader && parenStack->back() == 0)
    {
        BraceType returnVal = BraceType::DEFINITION_TYPE;
        if (foundNamespaceHeader)
            returnVal = returnVal | BraceType::NAMESPACE_TYPE;
        else if (foundClassHeader)
            returnVal = returnVal | BraceType::CLASS_TYPE;
        else if (foundInterfaceHeader)
            returnVal = returnVal | BraceType::INTERFACE_TYPE;
        return returnVal;
    }
    if (isInEnum)
        return BraceType::ARRAY_TYPE | BraceType::ENUM_TYPE;

    char prev = previousCommandChar;
    // a function or method body, a control statement, an arrow function,
    // a generic return type, an array return type, or an ambient module name
    if (prev == ')' || prev == '>' || prev == ']' || prev == '"')
        return BraceType::COMMAND_TYPE;
    // else, try, finally, do
    if (currentHeader != nullptr && isNonParenHeader)
        return BraceType::COMMAND_TYPE;
    // a block following a case label
    if (prev == ':' && (currentHeader == &ASResource::AS_CASE || currentHeader == &ASResource::AS_DEFAULT))
        return BraceType::COMMAND_TYPE;
    // a statement block
    if (prev == ';' || prev == '{' || prev == '}' || prev == ' ')
    {
        if (isBraceType(braceTypeStack->back(), BraceType::ARRAY_TYPE))
            return BraceType::ARRAY_TYPE;
        return BraceType::COMMAND_TYPE;
    }
    if (isLegalNameChar(prev))
    {
        // an object literal follows these keywords, a function body follows a return type
        std::string_view word = getPreviousCodeWord();
        if (word == "return" || word == "yield" || word == "await" || word == "default"
                || word == "export" || word == "import" || word == "typeof" || word == "void"
                || word == "in" || word == "of" || word == "case" || word == "throw"
                || word == "as" || word == "satisfies" || word == "extends" || word == "keyof")
            return BraceType::ARRAY_TYPE;
        return BraceType::COMMAND_TYPE;
    }
    return BraceType::ARRAY_TYPE;
...
/**
 * Get the brace type of an opening brace in Go, Rust, Kotlin, Swift and Dart.
 */
BraceType ASFormatter::getNewLanguageBraceType() const
{
    // type definitions
    if (foundPreDefinitionHeader && parenStack->back() == 0 && !isInTemplate)
    {
        BraceType returnVal = BraceType::DEFINITION_TYPE;
        if (foundNamespaceHeader)
            returnVal = returnVal | BraceType::NAMESPACE_TYPE;
        else if (foundStructHeader)
            returnVal = returnVal | BraceType::STRUCT_TYPE;
        else if (foundInterfaceHeader)
            returnVal = returnVal | BraceType::INTERFACE_TYPE;
        else
            returnVal = returnVal | BraceType::CLASS_TYPE;
    ...
    // a Go struct or interface type, an empty one is written as "interface{}"
    if (isGoStyle() && isLegalNameChar(previousCommandChar))
    {
        std::string_view typeWord = getPreviousCodeWord();
        if (typeWord == "struct" || typeWord == "interface")
        {
            // an empty or one-line type literal, e.g. "interface{}" or "struct{ A int }"
            if (peekNextChar() == '}' || isOneLineBlockReached(currentLine, charNum) != 0)
                return BraceType::ARRAY_TYPE | BraceType::INIT_TYPE;
            return BraceType::DEFINITION_TYPE
                   | (typeWord == "struct" ? BraceType::STRUCT_TYPE : BraceType::INTERFACE_TYPE);
        }
    }
    // a function body, e.g. "func f() error {" or "fn f() -> T where T: X {"
    if (functionBodyParenDepth >= 0 && functionBodyParenDepth == parenStack->back())
        return BraceType::COMMAND_TYPE;
    // the arms of a Rust match are like an initializer list
    if (matchBodyParenDepth >= 0 && matchBodyParenDepth == parenStack->back())
        return BraceType::ARRAY_TYPE;
    if (isInEnum && isRustStyle())
        return BraceType::ARRAY_TYPE | BraceType::ENUM_TYPE;

    char prev = previousCommandChar;
    // a Rust use tree, e.g. "use std::io::{self, Read};"
    if (isRustStyle() && prev == ':' && previousChar == ':')
        return BraceType::ARRAY_TYPE | BraceType::INIT_TYPE;
    // a control statement, a function body, or a trailing lambda
    if (prev == ')')
        return BraceType::COMMAND_TYPE;
    if (currentHeader != nullptr && isNonParenHeader)
        return BraceType::COMMAND_TYPE;
    ...
    if (isGoStyle() && prev == '}' && previousChar == '}')
        return BraceType::ARRAY_TYPE | BraceType::INIT_TYPE;
    // a statement block, a case block
    if (prev == ';' || prev == '{' || prev == '}' || prev == ':' || prev == ' ')
    {
        if (isBraceType(braceTypeStack->back(), BraceType::ARRAY_TYPE) && prev != ':')
            return BraceType::ARRAY_TYPE;
        return BraceType::COMMAND_TYPE;
    }

    std::string_view word = isLegalNameChar(prev) ? getPreviousCodeWord() : std::string_view();
    if (isGoStyle())
    {
        if (word == "else" || word == "select" || word == "go" || word == "defer")
            return BraceType::COMMAND_TYPE;
        // a composite literal, e.g. "T{a: 1}", "[]int{1, 2}", "map[K]V{}"
        if (isLegalNameChar(prev) || prev == ']')
            return BraceType::ARRAY_TYPE | BraceType::INIT_TYPE;
        return BraceType::ARRAY_TYPE;
    }
    if (isRustStyle())
    {
        // a macro transcriber with a block, e.g. "=> {{" ... "}};", the outer brace
        // stays with the inner one
        if (peekNextChar() == '{')
            return BraceType::ARRAY_TYPE | BraceType::INIT_TYPE;
        if (prev == '{' && previousChar == '{')
            return BraceType::COMMAND_TYPE;
        // a closure body, a match arm body, a block expression
        if (prev == '|' || prev == '>' || prev == '=')
            return BraceType::COMMAND_TYPE;
        if (word == "loop" || word == "unsafe" || word == "async" || word == "move" || word == "else"
                || word == "const" || word == "try")
            return BraceType::COMMAND_TYPE;
        // a struct literal, a macro invocation
        return BraceType::ARRAY_TYPE;
    }
    // Dart: a map or set literal follows these, otherwise a function body
    if (isLegalNameChar(prev)
            && !(word == "return" || word == "const" || word == "new" || word == "yield"
                 || word == "await" || word == "in" || word == "case" || word == "throw"))
        return BraceType::COMMAND_TYPE;
    if (prev == '*' || prev == '>')     // async*, sync*, or a generic return type
        return BraceType::COMMAND_TYPE;
    return BraceType::ARRAY_TYPE;
}

/**
 * Get the word preceding the current character, it may be on a previous line.
 */
std::string_view ASFormatter::getPreviousCodeWord() const
{
    std::string_view word = getPreviousWord(currentLine, charNum);
    if (!word.empty())
        return word;
    size_t firstText = currentLine.find_first_not_of(" \t");
    if (firstText != std::string::npos && (int) firstText < charNum)
        return word;
    // the word is on a previous line
    const std::string& line = formattedLine.find_first_not_of(" \t") != std::string::npos
                              ? formattedLine : readyFormattedLine;
    return getPreviousWord(line, (int) line.length());
}

/**
 * Split the brace types when a formatted line is split for max-code-length.
 * breakLine() has moved all of them to the ready line.
 */
void ASFormatter::splitLineBraceKinds()
{
    size_t braces = 0;
    bool inComment = false;
    for (size_t i = 0; i < readyFormattedLine.length(); i++)
    {
        if (inComment)
        {
            if (readyFormattedLine.compare(i, 2, "*/") == 0)
                inComment = false;
            continue;
        }
        if (readyFormattedLine.compare(i, 2, "//") == 0)
        ...
        if (readyFormattedLine.compare(i, 2, "/*") == 0)
            inComment = true;
        else if (readyFormattedLine[i] == '"')
        {
            size_t close = readyFormattedLine.find('"', i + 1);
    ...
    if (braces < readyFormattedLineBraceBlocks.size())
    {
        formattedLineBraceBlocks.insert(formattedLineBraceBlocks.begin(),
                                        readyFormattedLineBraceBlocks.begin() + braces,
                                        readyFormattedLineBraceBlocks.end());
        readyFormattedLineBraceBlocks.resize(braces);
    }
}
...
void ASFormatter::handleBreakLine()
{
            ...
            && !isLambdaSignatureLine)
    {
        ...
        {
            if (isBraceType(braceTypeStack->back(), BraceType::SINGLE_LINE_TYPE)
                    && (isBraceType(braceTypeStack->back(), BraceType::BREAK_BLOCK_TYPE)
                        || shouldBreakOneLineBlocks)
                    && !isBraceType(braceTypeStack->back(), BraceType::KEEP_ONE_LINE_TYPE))
                isInLineBreak = true;
    ...
    // a Scala expression continues after a block, e.g. "} yield x" or "} filter { ... }"
             ...
             && ((isLegalNameChar(currentChar) && currentChar != '.')
                 ...
                 || currentChar == '*'
                 || currentChar == '&'
                 // a closure that is called immediately, e.g. "}()" in Go
                 || (currentChar == '(' && !(isMaskedStyle() && previousChar == '}'))))
    {
        previousCommandChar = ' ';
        isInLineBreak = true;
    }
...
bool ASFormatter::handlePotentialHeader(const std::string *&newHeader)
{
    ...
    newHeader = findHeader(headers.get());

    // a keyword may be a property or a method name in JavaScript and TypeScript
    if (newHeader != nullptr && isJSMemberName(newHeader))
        newHeader = nullptr;

    // a Rust for loop has a condition in parens, virtual parens are added by ASLexer,
    // it is not the "for" of "impl Trait for Type"
    if (newHeader == &ASResource::AS_FOR && isRustStyle()
            && ASBase::peekNextChar(currentLine, charNum + 2) != '(')
        newHeader = nullptr;

    // a Swift enum case is not a switch case, a Swift else of a guard is not the else of an if
    if (newHeader == &ASResource::AS_CASE && isSwiftStyle()
            && isBraceType(braceTypeStack->back(), BraceType::DEFINITION_TYPE))
        newHeader = nullptr;
    if (newHeader == &ASResource::AS_ELSE && isSwiftStyle() && previousCommandChar != '}')
        newHeader = nullptr;

    // a C# switch expression is not a switch statement
    if (newHeader == &ASResource::AS_SWITCH && isSharpStyle() && isSharpSwitchExpression())
    {
        newHeader = nullptr;
        isSharpSwitchExpressionPending = true;
    }

    // a C# init accessor follows an opening brace, a semicolon, or an accessor modifier
    if (newHeader == &ASResource::AS_INIT
            && previousCommandChar != '{' && previousCommandChar != ';'
            && previousCommandChar != '}' && previousCommandChar != ']')
    {
        std::string_view prevWord = getPreviousCodeWord();
        if (prevWord != "private" && prevWord != "protected" && prevWord != "internal"
                && prevWord != "public" && prevWord != "readonly")
            newHeader = nullptr;
    }

    // java can have a 'default' not in a switch
    // a Java 'default' label may end with an arrow, e.g. "default -> x();"
    if (newHeader == &ASResource::AS_DEFAULT
            && ASBeautifier::peekNextChar(
                currentLine, charNum + (*newHeader).length() - 1) != ':')
    {
        size_t next = currentLine.find_first_not_of(" \t", charNum + newHeader->length());
        if (!isJavaStyle() || next == std::string::npos || currentLine.compare(next, 2, "->") != 0)
            newHeader = nullptr;
    }
            ...
            && (newHeader == &ASResource::AS_SYNCHRONIZED))
    {
        ...
        if (!isBraceType(braceTypeStack->back(), BraceType::COMMAND_TYPE))
            newHeader = nullptr;
    }
    ...
    if (newHeader != nullptr)
    {
        ...
        {
            // a closing header following a block kept on one line is not broken,
            // e.g. "if c { a } else { b }"
            if (isBraceType(previousBraceType, BraceType::KEEP_ONE_LINE_TYPE) && !isInLineBreak)
                appendSpacePad();
            else if (isOkToBreakBlock(braceTypeStack->back()))
        ...
        isNonParenHeader = findHeader(nonParenHeaders.get()) != nullptr;
    ...
    if ((newHeader = findHeader(preDefinitionHeaders.get())) != nullptr
            && parenStack->back() == 0
            && !isInEnum		// not C++11 enum class
            && !isJSMemberName(newHeader, true)
            && !(newHeader == &ASResource::AS_RECORD && !isSharpRecordHeader()))
    {
        if (newHeader == &ASResource::AS_NAMESPACE || newHeader == &ASResource::AS_MODULE)
            foundNamespaceHeader = true;
        if (newHeader == &ASResource::AS_CLASS || newHeader == &ASResource::AS_RECORD)
    ...
    if ((newHeader = findHeader(preCommandHeaders.get())) != nullptr)
    {
        // must be after function arguments
        if (previousNonWSChar == ')')
            foundPreCommandHeader = true;
    }
    else if ((newHeader = findHeader(castOperators.get())) != nullptr)
    {
...
void ASFormatter::handleEndOfBlock()
{
    if (currentChar == ';' && !isInAsmBlock)
    {
        ...
        if (((shouldBreakOneLineStatements
                || isBraceType(braceTypeStack->back(), BraceType::SINGLE_LINE_TYPE))
                && isOkToBreakBlock(braceTypeStack->back()))
...
void ASFormatter::handleColonSection()
{
    ...
    {
        isInCase = false;
        // gofmt and swift-format keep a statement on the line of the case label
        if (shouldBreakOneLineStatements && !isGoStyle() && !isSwiftStyle())
            passedColon = true;
    }
...
{
    // a function keyword, the next brace at its paren depth is the function body
    // a Rust match, the next brace at its paren depth contains the match arms
    if (isNewLanguageStyle())
    {
        std::string_view word = getCurrentWord(currentLine, charNum);
        if (word == "func" || word == "fn" || word == "fun")
            functionBodyParenDepth = parenStack->back();
        else if (word == "match" && isRustStyle())
            matchBodyParenDepth = parenStack->back();
    }
    ...
    if (isCStyle() && findKeyword(currentLine, charNum, ASResource::AS_AUTO)
            && (isBraceType(braceTypeStack->back(), BraceType::NULL_TYPE)
                || isBraceType(braceTypeStack->back(), BraceType::DEFINITION_TYPE))
    ...
    {
        if ((isBraceType(braceTypeStack->back(), BraceType::NULL_TYPE)
                || isBraceType(braceTypeStack->back(), BraceType::DEFINITION_TYPE))
                && !returnTypeChecked
...
void ASFormatter::formatFirstOpenBrace(BraceType braceType)
{
    ...
    // JavaScript object literals and arrays are always attached,
    // a line break after 'return' would end the statement
    BraceMode braceMode = braceFormatMode;
    if (isJSStyle() && (braceMode == BREAK_MODE || braceMode == RUN_IN_MODE))
        braceMode = ATTACH_MODE;
    // an element of an array or an argument list is not attached to the previous element
    if (isJSStyle() && currentLineBeginsWithBrace && (previousCommandChar == ',' || previousCommandChar == '['))
    {
        appendCurrentChar();
        return;
    }
    ...
    if (braceMode == ATTACH_MODE || braceMode == LINUX_MODE)
    {
        ...
        if (isBraceType(braceType, BraceType::ENUM_TYPE)
        ...
        else if (isCharImmediatelyPostLineComment && !isBraceType(braceType, BraceType::SINGLE_LINE_TYPE))
        {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    if (isPaddedBrace || !isMaskedStyle())
                        appendSpacePad();
                ...
                else
                {
                    if (isPaddedBrace)
                    {
                        ...
                        if (!isBraceType(braceType, BraceType::INIT_TYPE))
                            appendSpacePad();
                    }
                    appendCurrentChar();
                }
    ...
    else if (braceMode == BREAK_MODE)
    {
        ...
        if (!isInLineBreak && isPaddedBrace)
        {
            // don't space pad C++11 uniform initialization
            if (!isBraceType(braceType, BraceType::INIT_TYPE))
                appendSpacePad();
...
void ASFormatter::formatCloseBrace(BraceType braceType)
{
    ...
    else
    {
        ...
        // a JavaScript object keeps its closing brace on the line of the input
        bool isJSBraceAttached = isJSStyle()
                                 && currentLine.find_first_not_of(" \t") != static_cast<size_t>(charNum);
        if (!isBraceType(braceType, BraceType::INIT_TYPE)
                && (!isBraceType(braceType, BraceType::SINGLE_LINE_TYPE)
...
std::string ASFormatter::nextLine()
{
    ...
    while (!isLineReady)
    {
        ...
        else // stuff to do when reading a new character...
        {
            ...
            if (isInClassInitializer
                    && isBraceType(braceTypeStack->back(), BraceType::COMMAND_TYPE))
                isInClassInitializer = false;
            if (isInBraceRunIn)
                isInLineBreak = false;
        ...
        {
            // a statement beginning with a literal, the line break after the previous
            // statement is done before the literal, e.g. "x();" and "\"a\".len();"
            if (passedSemicolon && isMaskedStyle())
            {
        ...
        if (currentChar == '#' && !isBraceType(braceTypeStack->back(), BraceType::SINGLE_LINE_TYPE))
        {
        ...
        if (methodBreakCharNum.has_value() && (size_t) charNum >= *methodBreakCharNum && methodBreakLineNum == 0)
        {
        ...
        if (methodAttachCharNum.has_value() && (size_t) charNum >= *methodAttachCharNum && methodAttachLineNum == 0)
        {
        ...
        if ((((previousCommandChar == '{' && isPreviousBraceBlockRelated)
                || ((previousCommandChar == '}'
                     ...
                     && peekNextChar() != ' '
                     && !isBraceType(previousBraceType, BraceType::DEFINITION_TYPE)
                     && !(isBraceType(previousBraceType, BraceType::KEEP_ONE_LINE_TYPE) && !isSharpStyle()))
                    && !isBraceType(braceTypeStack->back(), BraceType::DEFINITION_TYPE)))
                && isOkToBreakBlock(braceTypeStack->back()))
                // check for array
                || (previousCommandChar == '{'			// added 9/30/2010
                    && isBraceType(braceTypeStack->back(), BraceType::ARRAY_TYPE)
                    && !isBraceType(braceTypeStack->back(), BraceType::SINGLE_LINE_TYPE)
                    && isNonInStatementArray
                    // a JavaScript object keeps the text after its brace, it is idempotent
                    && !isJSStyle())
        ...
        // the end of an object literal or an initializer does not end a statement
        if ((previousNonWSChar == '}' && (isPreviousBraceBlockRelated || !isMaskedStyle()))
                || currentChar == ';')
        {
            handleEndOfBlock();
        }
        ...
        if (currentChar == ':'
                ...
                && !(isGoStyle() && peekNextChar() == '='))    // not a Go ':='
        {
            handleColonSection();
        }

        if (currentChar == '?' && isTernaryQuestion())
        ...
        if ((currentChar == '-' || currentChar == '+')
                ...
                && !isInObjCMethodDefinition
                && (isBraceType(braceTypeStack->back(), BraceType::NULL_TYPE)
                    || (isBraceType(braceTypeStack->back(), BraceType::EXTERN_TYPE))))
        {
        ...
        {
            newHeader = findOperator(operators.get());
    ...
    bool isInNamespace = isBraceType(braceTypeStack->back(), BraceType::NAMESPACE_TYPE);
    ...
    if (prependEmptyLine		// prepend a blank line before this formatted line
            ...
            && (!previousReadyFormattedLineLength.has_value() || *previousReadyFormattedLineLength > 0))
    {
        isLineReady = true;		// signal a waiting readyFormattedLine
        lineBraceIsBlock.clear();
        lineBraceIndex = 0;
    ...
    else		// format the current formatted line
    {
        ...
        lineBraceIsBlock = readyFormattedLineBraceBlocks;
        lineBraceIndex = 0;
...
{
    if (a == BraceType::NULL_TYPE || b == BraceType::NULL_TYPE)
        return (a == b);
    return ((a & b) == b);
}
...
bool ASFormatter::getNextLine(bool emptyLineWasDeleted /*false*/)
{
    ...
    if (shouldDeleteEmptyLines
            && lineIsEmpty
            && isBraceType((*braceTypeStack)[braceTypeStack->size() - 1], BraceType::COMMAND_TYPE))
    {
...
void ASFormatter::breakLine(bool isSplitLine /*false*/)
{
    ...
    formattedLine.erase();
    readyFormattedLineBraceBlocks = std::move(formattedLineBraceBlocks);
    formattedLineBraceBlocks.clear();
...
BraceType ASFormatter::getBraceType()
{
    ...
    BraceType returnVal = BraceType::NULL_TYPE;

    if (isJSStyle())
        returnVal = getJSBraceType();
    else if (isNewLanguageStyle())
        returnVal = getNewLanguageBraceType();
    // the arms of a C# switch expression are like an initializer list
    else if (isSharpSwitchExpressionPending)
        returnVal = BraceType::ARRAY_TYPE;
    ...
    {
        returnVal = BraceType::DEFINITION_TYPE;
        if (foundNamespaceHeader)
            returnVal = returnVal | BraceType::NAMESPACE_TYPE;
        else if (foundClassHeader)
            returnVal = returnVal | BraceType::CLASS_TYPE;
    ...
    {
        returnVal = BraceType::ARRAY_TYPE | BraceType::ENUM_TYPE;
    }
    else if (isSharpStyle() &&
             parenStack->back() == 0 &&
    ...
    // a Java lambda body
    else if (isJavaStyle() && isAfterLambdaArrow())
    {
        returnVal = BraceType::COMMAND_TYPE;
    }
    // a C# object initializer or a 'with' expression, but not a lambda body
    else if (isSharpStyle()
             && !isAfterLambdaArrow()
             && (isInAllocator || getPreviousCodeWord() == "with"))
    {
        returnVal = BraceType::ARRAY_TYPE;
    }
    else
    {
        ...
        if (isInExternC)
            returnVal = (isCommandType ? BraceType::COMMAND_TYPE : BraceType::EXTERN_TYPE);
        else
            returnVal = (isCommandType ? BraceType::COMMAND_TYPE : BraceType::ARRAY_TYPE);
    }

    int foundOneLineBlock = isOneLineBlockReached(currentLine, charNum);

    // a JavaScript function or statement body is broken as Prettier does, also in
    // a one-line object literal, so the next formatting does not break it
    bool isJSBodyBroken = isJSStyle() && returnVal == BraceType::COMMAND_TYPE
                          && (foundOneLineBlock == 1 || foundOneLineBlock == 2) && shouldBreakOneLineBlocks;
    if (foundOneLineBlock == 2 && returnVal == BraceType::COMMAND_TYPE && !isJSBodyBroken)
        returnVal = BraceType::ARRAY_TYPE;

    if (foundOneLineBlock > 0)
    {
        returnVal = returnVal | BraceType::SINGLE_LINE_TYPE;
        if (breakCurrentOneLineBlock || isJSBodyBroken)
        ...
        // or a Dart closure in the arguments of a call
                 ...
                 && ((isRustStyle()
                      && (parenStack->back() > 0 || isInPotentialCalculation
                          || (isOneLineBlockExpression()
                              && (getPreviousCodeWord() == "unsafe" || previousCommandChar == '|'))
                          || isBraceType(braceTypeStack->back(), BraceType::ARRAY_TYPE)))
                     || (isDartStyle() && parenStack->back() > 0))
                 && !isBraceType(returnVal, BraceType::DEFINITION_TYPE)
                 && !(functionBodyParenDepth >= 0 && functionBodyParenDepth == parenStack->back()))
            returnVal = returnVal | BraceType::KEEP_ONE_LINE_TYPE;
        // C# auto-properties and simple accessors are kept on one line
        else if (foundOneLineBlock == 1 && isSharpStyle() && !breakCurrentOneLineBlock
                 && isBraceType(returnVal, BraceType::COMMAND_TYPE)
                 && isSharpSimpleAccessorBlock())
            returnVal = returnVal | BraceType::KEEP_ONE_LINE_TYPE;
    }

    if (isBraceType(returnVal, BraceType::ARRAY_TYPE))
    {
        if (isNonInStatementArrayBrace())
        {
            returnVal = returnVal | BraceType::ARRAY_NIS_TYPE;
            isNonInStatementArray = true;
            isImmediatelyPostNonInStmt = false;		// in case of "},{"
            if (isMaskedStyle())
                nonInStatementArrayDepths.push_back(braceTypeStack->size() + 1);
            nonInStatementBrace = formattedLine.length() - 1;
        }
        if (isUniformInitializerBrace())
            returnVal = returnVal | BraceType::INIT_TYPE;
    }
...
bool ASFormatter::isPointerOrReference() const
{
    ...
    std::string_view lastWord = getPreviousWord(currentLine, charNum);
    if (lastWord.empty())
        lastWord = " ";
    ...
    if (currentChar == '&' && nextChar == '&')
    {
        ...
        if (currentHeader != nullptr || isInPotentialCalculation)
            return false;
        if (parenStack->back() > 0 && isBraceType(braceTypeStack->back(), BraceType::COMMAND_TYPE))
            return false;
        return true;
    }
...
bool ASFormatter::isUnaryOperator() const
{
    ...
    if (previousCommandChar == ')')
    {
        ...
        std::string_view prevWord = getPreviousWord(currentLine, end);
        if (prevWord.empty())
            return false;
...
{
    assert(currentChar == '+' || currentChar == '-');
    std::string_view prevWord = getPreviousWord(currentLine, charNum, true);

    if (charNum && !prevWord.empty() && isDigit(prevWord[0]))
    {
...
bool ASFormatter::isNextWordSharpNonParenHeader(int startChar) const
{
    ...
    if (!isCharPotentialHeader(nextText, 0))
        return false;
    // bypass the accessor modifiers
    size_t start = 0;
    for (;;)
    {
        std::string_view word = getCurrentWord(nextText, start);
        if (word != "private" && word != "protected" && word != "internal" && word != "public" && word != "readonly")
            break;
        start = nextText.find_first_not_of(" \t", start + word.length());
        if (start == std::string::npos || !isCharPotentialHeader(nextText, start))
            return false;
    }
    if (findKeyword(nextText, start, ASResource::AS_GET) || findKeyword(nextText, start, ASResource::AS_SET)
            || findKeyword(nextText, start, ASResource::AS_INIT)
            || findKeyword(nextText, start, ASResource::AS_ADD) || findKeyword(nextText, start, ASResource::AS_REMOVE))
        return true;
    return false;
}

/**
 * Check if the one-line block at the current opening brace is a simple C#
 * accessor block that is kept on one line (as Rider and Visual Studio do):
 * an accessor list without accessor bodies, e.g. "{ get; private set; }" or
 * "{ get => x; init => x = value; }", or an accessor body with a single
 * statement, e.g. "get { return x; }".
 */
bool ASFormatter::isSharpSimpleAccessorBlock() const
{
    assert(currentChar == '{');
    bool isAccessorBody = (currentHeader == &ASResource::AS_GET || currentHeader == &ASResource::AS_SET
                           || currentHeader == &ASResource::AS_INIT || currentHeader == &ASResource::AS_ADD
                           || currentHeader == &ASResource::AS_REMOVE);
    if (!isAccessorBody && !isNextWordSharpNonParenHeader(charNum + 1))
        return false;
    // the block must not contain other blocks, and an accessor body is a single statement
    int statements = 0;
    size_t itemStart = charNum + 1;
    for (size_t i = charNum + 1; i < currentLine.length(); i++)
    {
        char ch = currentLine[i];
        if (ch == '{' || currentLine.compare(i, 2, "//") == 0 || currentLine.compare(i, 2, "/*") == 0)
            return false;
        if (ch == '"')
        {
            size_t close = currentLine.find('"', i + 1);
            if (close == std::string::npos)
                return false;
            i = close;
            continue;
        ...
        if (ch == ';' || ch == '}')
        {
            // every item of an accessor list must be an accessor
            if (!isAccessorBody
                    && !isSharpAccessorDeclaration(std::string_view(currentLine).substr(itemStart, i - itemStart)))
                return false;
            itemStart = i + 1;
        }
        if (ch == ';')
            ++statements;
        if (ch == '}')
            return !isAccessorBody || statements <= 1;
    }
    return false;
}

/**
 * Check if the current '?' is a conditional operator, not a part of a type or
 * an optional marker, e.g. "int?", "String?", "a?: T", "(a?)".
 * Rust and Kotlin have no conditional operator.
 */
bool ASFormatter::isTernaryQuestion() const
{
    ...
    if (isSwiftStyle() || isDartStyle())
    {
        if (next == '.' || next == '[' || next == ')' || next == ',' || next == '>' || next == ';' || next == '=')
            return false;
        // an optional type is attached to the type name
        if (charNum > 0 && !std::isblank(currentLine[charNum - 1])
...
/**
 * Operators that are never padded: optional chaining, ranges, spreads,
 * the Kotlin not-null assertion, and the Go channel operator.
 */
bool ASFormatter::isNeverPaddedOperator(const std::string* newOperator) const
{
    return newOperator == &ASResource::AS_QUESTION_DOT
           || newOperator == &ASResource::AS_ELLIPSIS
           || newOperator == &ASResource::AS_RANGE
           || newOperator == &ASResource::AS_RANGE_INCLUSIVE
           || newOperator == &ASResource::AS_RANGE_EXCLUSIVE
           || newOperator == &ASResource::AS_NOT_NOT
           || newOperator == &ASResource::AS_CHANNEL
           || newOperator == &ASResource::AS_NULL_CASCADE
           || newOperator == &ASResource::AS_NULL_SPREAD;
}

/**
 * Check if a '*' is the star of a JavaScript generator,
 * e.g. "function* g()", "yield* other", "*gen() {".
 */
bool ASFormatter::isGeneratorStar() const
{
    if (!isJSStyle())
        return false;
    std::string_view prevWord = getPreviousWord(currentLine, charNum);
    if (prevWord == "function" || prevWord == "yield" || prevWord == "async" || prevWord == "static")
        return true;
    char prev = previousNonWSChar;
    return prev == '{' || prev == '}' || prev == ';' || prev == ',' || (int) currentLine.find_first_not_of(" \t") == charNum;
}

/**
 * Check if the parameters of a Kotlin lambda or a Swift closure follow the
 * current opening brace, e.g. "{ item ->" or "{ data, error in".
 */
bool ASFormatter::isLambdaSignatureFollows() const
{
    ...
    for (size_t i = charNum + 1; i < currentLine.length(); i++)
    {
        ...
        if (ch == '(' || ch == '[' || ch == '<')
            ++depth;
        else if (ch == ')' || ch == ']' || (ch == '>' && currentLine[i - 1] != '-'))
            --depth;
        else if (ch == '{' || ch == '}' || currentLine.compare(i, 2, "//") == 0)
            return false;
        else if (depth == 0 && isKotlinStyle() && currentLine.compare(i, 2, "->") == 0)
            return true;
        else if (depth == 0 && isSwiftStyle() && currentLine.compare(i, 2, "in") == 0
                 && !isLegalNameChar(currentLine[i - 1])
                 && (i + 2 == currentLine.length() || !isLegalNameChar(currentLine[i + 2])))
            return true;
    }
    return false;
}
...
/**
 * Check if the one-line block at the current opening brace contains an
 * expression without a statement, e.g. the Rust "unsafe { f() }".
 */
bool ASFormatter::isOneLineBlockExpression() const
{
    assert(currentChar == '{');
    int depth = 0;
...
/**
 * Check if the code preceding the current character ends with a lambda arrow,
 * '=>' in C#, or '->' in Java.
 */
bool ASFormatter::isAfterLambdaArrow() const
{
    const std::string& line = formattedLine.find_first_not_of(" \t") != std::string::npos
                              ? formattedLine : readyFormattedLine;
    size_t end = line.find_last_not_of(" \t");
    if (end == std::string::npos || end < 1 || line[end] != '>')
        return false;
    return line[end - 1] == (isJavaStyle() ? '-' : '=');
}

/**
 * Check if the text is a C# accessor declaration without a body,
 * e.g. "get", "private set", "[Attr] init", or "get => x".
 * Empty text is accepted, it is the text after the last semicolon.
 */
bool ASFormatter::isSharpAccessorDeclaration(std::string_view text) const
{
    ...
    while (i < text.length() && text[i] == '[')
    {
        ...
        if (close == std::string_view::npos)
            return false;
        i = text.find_first_not_of(" \t", close + 1);
        if (i == std::string_view::npos)
            return false;
    }
    for (;;)
    {
        if (!isCharPotentialHeader(text, i))
            return false;
        std::string_view word = getCurrentWord(text, i);
        i = text.find_first_not_of(" \t", i + word.length());
        if (word == "private" || word == "protected" || word == "internal" || word == "public" || word == "readonly")
        {
            if (i == std::string_view::npos)
                return false;
        ...
        if (word != "get" && word != "set" && word != "init" && word != "add" && word != "remove")
            return false;
        // an auto accessor, or an expression bodied accessor
        return i == std::string_view::npos || text.compare(i, 2, "=>") == 0;
    }
}

/**
 * Check if the current 'switch' is a C# switch expression, e.g. "x switch { ... }".
 * A switch expression follows an expression and is followed by a brace.
 */
bool ASFormatter::isSharpSwitchExpression() const
{
    char prev = previousCommandChar;
    if (!(isLegalNameChar(prev) || prev == ')' || prev == ']' || prev == '"'))
        return false;
    std::string nextText = peekNextText(currentLine.substr(charNum + ASResource::AS_SWITCH.length()));
    return !nextText.empty() && nextText[0] == '{';
}

/**
 * Check if the current 'record' begins a C# record declaration.
 * 'record' may also be an identifier.
 */
bool ASFormatter::isSharpRecordHeader() const
{
    size_t next = currentLine.find_first_not_of(" \t", charNum + ASResource::AS_RECORD.length());
    if (next == std::string::npos || next == (size_t) charNum + ASResource::AS_RECORD.length())
        return false;
    if (!isCharPotentialHeader(currentLine, next))
        return false;
    std::string_view word = getCurrentWord(currentLine, next);
    return !(word == "is" || word == "as" || word == "in" || word == "and" || word == "or"
             || word == "not" || word == "with" || word == "switch" || word == "when");
}
...
void ASFormatter::padOperators(const std::string* newOperator)
{
    ...
    // a '?' that is not a conditional operator, e.g. an optional or nullable type
    bool isTypeQuestion = (newOperator == &ASResource::AS_QUESTION && !isSharpStyle() && !isTernaryQuestion());
    ...
    bool shouldPad = (newOperator != &ASResource::AS_SCOPE_RESOLUTION &&
                      ...
                      !isTypeQuestion &&
                      !isNeverPaddedOperator(newOperator) &&
                      !(newOperator == &ASResource::AS_MULT && isGeneratorStar()) &&
                      !isCharImmediatelyPostOperator &&
                      !isInCase &&
                      !isInAsm &&
                      !isInAsmOneLine &&
        ...
        if (shouldUnPad && !isInStruct)
        {
            ...
            if (i != std::string::npos)
            {
                ...
                else
                {
                    ...
                    std::string_view prevWord = getPreviousWord(formattedLine, formattedLine.length());
                    const std::string* prevWordH = nullptr;
                    if (shouldPadHeader
                            && !prevWord.empty()
                            && isCharPotentialHeader(prevWord, 0))
                        prevWordH = ASBase::findHeader(prevWord, 0, headers.get());
...
{
    assert(!isBraceType(braceType, BraceType::ARRAY_TYPE));
    assert(currentChar == '{');
    ...
    if (breakBrace)
    {
        ...
        else if (!isBraceType(braceType, BraceType::SINGLE_LINE_TYPE))
        {
            formattedLine = rtrim(formattedLine);
            breakLine();
        }
        else if ((shouldBreakOneLineBlocks || isBraceType(braceType, BraceType::BREAK_BLOCK_TYPE))
                 && !isBraceType(braceType, BraceType::EMPTY_BLOCK_TYPE)
                 && !isBraceType(braceType, BraceType::KEEP_ONE_LINE_TYPE))
            breakLine();
        else if (!isInLineBreak)
            appendSpacePad();
            ...
            if (isEmptyLine(formattedLine))
                appendCurrentChar();				// don't attach
                     ...
                     && !(isImmediatelyPostPreprocessor
                          && currentLineBeginsWithBrace))
            {
                if (!isBraceType(braceType, BraceType::EMPTY_BLOCK_TYPE))
                {
                    appendSpacePad();
                    appendCurrentChar(false);				// OK to attach
                    testForTimeToSplitFormattedLine();		// line length will have changed
                    ...
                    // the parameters of a Kotlin lambda or a Swift closure stay with the brace,
                    // e.g. "items.forEach { item ->"
                    else if (!isBeforeAnyComment() && !isLambdaSignatureFollows())		// added in release 2.03
                    {
                        shouldBreakLineAtNextChar = true;
                    }
                }
...
void ASFormatter::formatClosingBrace(BraceType braceType)
{
    ...
    if (isBraceType(braceType, BraceType::DEFINITION_TYPE)
            && !isBraceType(braceType, BraceType::NAMESPACE_TYPE))
    {
    ...
    if (attachClosingBraceMode)
    {
        ...
        if ((isEmptyLine(formattedLine)			// if a blank line precedes this
                ...
                && (!isBraceType(braceType, BraceType::SINGLE_LINE_TYPE) || isOkToBreakBlock(braceType)))
        {
            breakLine();
            appendCurrentChar();				// don't attach
        }
    ...
    else if (!isBraceType(braceType, BraceType::EMPTY_BLOCK_TYPE)
             && (isBraceType(braceType, BraceType::BREAK_BLOCK_TYPE)
                 || isOkToBreakBlock(braceType)))
    {
        breakLine();
        appendCurrentChar();
    ...
    // a member access follows a closure, e.g. "items.map { x }.sum()"
    if (isLegalNameChar(peekNextChar()) && !(isMaskedStyle() && peekNextChar() == '.'))
        appendSpaceAfter();
    ...
    // line-between-members: insert blank after top-level function close
             ...
             && (isBraceType(braceTypeStack->back(), BraceType::NULL_TYPE)
                 || isBraceType(braceTypeStack->back(), BraceType::NAMESPACE_TYPE))
             && !isBraceType(braceType, BraceType::DEFINITION_TYPE)
             && !isBraceType(braceType, BraceType::NAMESPACE_TYPE))
    {
        isAppendPostBlockEmptyLineRequested = true;
    }
 ...
 * the calling function should have a continue statement after calling this method
 *
 * @param braceType            the type of brace to be formatted, must be an BraceType::ARRAY_TYPE.
...
void ASFormatter::formatRunIn()
{
    ...
    if (isBraceType(braceTypeStack->back(), BraceType::NAMESPACE_TYPE))
        return; // false;
    ...
    if (isCStyle()
            ...
            && (isBraceType(braceTypeStack->back(), BraceType::CLASS_TYPE)
                || (isBraceType(braceTypeStack->back(), BraceType::STRUCT_TYPE)
...
{
    assert(isBraceType(braceTypeStack->back(), BraceType::ARRAY_TYPE));
...
{
    if (isBraceType(braceType, BraceType::KEEP_ONE_LINE_TYPE))
        return false;
    // Actually, there should not be an BraceType::ARRAY_TYPE brace here.
    ...
    if (isBraceType(braceType, BraceType::ARRAY_TYPE)
            && isBraceType(braceType, BraceType::SINGLE_LINE_TYPE))
        return false;
    if (isBraceType(braceType, BraceType::COMMAND_TYPE)
            && isBraceType(braceType, BraceType::EMPTY_BLOCK_TYPE))
        return false;
    if (!isBraceType(braceType, BraceType::SINGLE_LINE_TYPE)
            || isBraceType(braceType, BraceType::BREAK_BLOCK_TYPE)
            || shouldBreakOneLineBlocks)
        return true;
    return false;
...
/**
 * In JavaScript and TypeScript a keyword may be the name of a property or a
 * method. It is a name if it is followed by a colon, or if it is directly in an
 * object literal or a class body, where there are no statements.
 */
bool ASFormatter::isJSMemberName(const std::string* header, bool isDefinition) const
{
    if (!isJSStyle())
        return false;
    size_t next = currentLine.find_first_not_of(" \t", charNum + header->length());
    if (next != std::string::npos
            && ((currentLine[next] == ':' && currentLine.compare(next, 2, "::") != 0)
                || currentLine.compare(next, 2, "?:") == 0)
            && header != &ASResource::AS_DEFAULT && header != &ASResource::AS_CASE)
        return true;
    if (parenStack->back() != 0)
        return false;
    // a definition may be nested in a class or a namespace
    return isBraceType(braceTypeStack->back(), BraceType::ARRAY_TYPE)
           || (!isDefinition && isBraceType(braceTypeStack->back(), BraceType::DEFINITION_TYPE));
}
...
const std::string* ASFormatter::checkForHeaderFollowingComment(std::string_view firstLine) const
{
    ...
    return ASBase::findHeader(nextText, 0, headers.get());
}
...
void ASFormatter::processPreprocessor()
{
    ...
    if (shouldIndentPreprocBlock
            && (isBraceType(braceTypeStack->back(), BraceType::NULL_TYPE) || isBraceType(braceTypeStack->back(), BraceType::NAMESPACE_TYPE))
            && !foundClassHeader
            && !isInClassInitializer
...
bool ASFormatter::commentAndHeaderFollows()
{
    ...
    const std::string* newHeader = ASBase::findHeader(nextText, 0, headers.get());
...
bool ASFormatter::isCurrentBraceBroken() const
{
    ...
    if (shouldAttachExternC
            && isBraceType((*braceTypeStack)[stackEnd], BraceType::EXTERN_TYPE))
    {
        return false;
    }
    if (shouldAttachNamespace
            && isBraceType((*braceTypeStack)[stackEnd], BraceType::NAMESPACE_TYPE))
    {
        return false;
    }
    if (shouldAttachClass
            && (isBraceType((*braceTypeStack)[stackEnd], BraceType::CLASS_TYPE)
                || isBraceType((*braceTypeStack)[stackEnd], BraceType::INTERFACE_TYPE)))
    {
        return false;
    }
    if (shouldAttachInline
            ...
            && isBraceType((*braceTypeStack)[stackEnd], BraceType::COMMAND_TYPE))
    {
        size_t i;
        for (i = 1; i < braceTypeStack->size(); i++)
            if (isBraceType((*braceTypeStack)[i], BraceType::CLASS_TYPE)
                    || isBraceType((*braceTypeStack)[i], BraceType::STRUCT_TYPE))
                return false;
    }

    // check braces
    if (isBraceType((*braceTypeStack)[stackEnd], BraceType::EXTERN_TYPE))
    {
    ...
    {
        // break a namespace
        if (isBraceType((*braceTypeStack)[stackEnd], BraceType::NAMESPACE_TYPE))
        {
        ...
        else if (isBraceType((*braceTypeStack)[stackEnd], BraceType::CLASS_TYPE)
                 || isBraceType((*braceTypeStack)[stackEnd], BraceType::INTERFACE_TYPE))
        {
            if (formattingStyle != STYLE_STROUSTRUP
                    && formattingStyle != STYLE_WEBKIT)
        ...
        else if (isBraceType((*braceTypeStack)[stackEnd], BraceType::STRUCT_TYPE))
        {
        ...
        else if (isBraceType((*braceTypeStack)[stackEnd], BraceType::COMMAND_TYPE))
        {
            if (stackEnd == 1)
            {
                breakBrace = true;
            ...
            else if (stackEnd > 1)
            {
                ...
                if (isBraceType((*braceTypeStack)[stackEnd - 1], BraceType::NAMESPACE_TYPE)
                        || isBraceType((*braceTypeStack)[stackEnd - 1], BraceType::CLASS_TYPE)
                        || (isBraceType((*braceTypeStack)[stackEnd - 1], BraceType::ARRAY_TYPE) && !lambdaIndicator)
                        || isBraceType((*braceTypeStack)[stackEnd - 1], BraceType::STRUCT_TYPE)
                        || isBraceType((*braceTypeStack)[stackEnd - 1], BraceType::EXTERN_TYPE)
                   )
                {
                    breakBrace = true;
                }
 ...
 */
std::string_view ASFormatter::getPreviousWord(const std::string& line, int currPos, bool allowDots) const
{
    // get the last legal word (may be a number)
    if (currPos == 0)
    ...
    return std::string_view(line).substr(start, end - start + 1);
}
...
bool ASFormatter::addBracesToStatement()
{
    ...
    if (shouldAddBraces)
    {
        ...
        if (isCharPotentialHeader(currentLine, charNum))
            if (findHeader(headers.get()) != nullptr)
                return false;
...
bool ASFormatter::removeBracesFromStatement()
{
    ...
    if ((nextLine_.compare(nextChar, 2, "/*") == 0)
            ...
            || (isCharPotentialHeader(nextLine_, nextChar)
                && ASBase::findHeader(nextLine_, nextChar, headers.get()) != nullptr))
        return false;
...
{
    assert((isBraceType(braceTypeStack->back(), BraceType::NULL_TYPE)
            || isBraceType(braceTypeStack->back(), BraceType::DEFINITION_TYPE)));
        ...
        for (size_t i = 0; i < line.length(); i++)
        {
            ...
            if (line[i] == '{')
            {
                ...
                if (shouldAttachReturnType && foundSplitPoint && isAlreadyBroken)
                {
                    ...
                    if ((maxCodeLength != std::string::npos && previousReadyFormattedLineLength.has_value() && *previousReadyFormattedLineLength < maxCodeLength) || maxCodeLength == std::string::npos)
                    {
...
void ASFormatter::checkIfTemplateOpener()
{
    ...
    templateDepth = 0;
    // the type arguments of these languages may contain union and function types,
    // object types, optional types, and string literal types
    bool isModernGeneric = isJSStyle() || isNewLanguageStyle();
    ...
    while (stream.hasMoreLines() || isFirstLine)
    {
        ...
        for (size_t i = 0; i < nextLine_.length(); i++)
        {
            ...
            if (currentChar_ == '>')
            {
                // the '>' of a function type arrow '=>' or '->'
                if (isModernGeneric && i > 0 && (nextLine_[i - 1] == '=' || nextLine_[i - 1] == '-'))
                    continue;
                --templateDepth;
                if (templateDepth == 0)
                {
            ...
            if (isModernGeneric)
            {
                if (currentChar_ == '|' || currentChar_ == '?' || currentChar_ == '.' || currentChar_ == '-'
                        || currentChar_ == '+' || currentChar_ == '!' || currentChar_ == '"' || currentChar_ == '\''
                        || currentChar_ == '\x1C' || currentChar_ == '\x1D')
                    continue;
                if (currentChar_ == '{')
                {
                    ++braceDepth_;
                ...
                if ((currentChar_ == ';' || currentChar_ == ',') && braceDepth_ > 0)
                    continue;
            }
...
bool ASFormatter::isOkToSplitFormattedLine()
{
    ...
    {
        shouldKeepLineUnbroken = true;
        if (!isBraceType(braceTypeStack->back(), BraceType::ARRAY_NIS_TYPE))
            clearFormattedLineSplitPoints();
        return false;
    }
    return true;
...
const std::string* ASFormatter::getFollowingOperator() const
{
    ...
    const std::string* newOperator = ASBase::findOperator(currentLine, nextNum, operators.get());
    return newOperator;
}
