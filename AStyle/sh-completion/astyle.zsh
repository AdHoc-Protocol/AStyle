...
_astyle() {
  local -a options=(
    "--style[Set a formatting and indenting style]: :(allman java kr stroustrup whitesmith vtk ratliff gnu linux horstmann 1tbs google mozilla webkit pico lisp none)"
    "--indent[Set indentation option]: :(spaces tab force-tab force-tab-x none)"
    ...
    "--pad-negation[Insert space padding after negations]: :(before)"
    "--pad-type-colon[Scala: set the space padding around a type colon]: :(after all none)"
    "--align-declarations[Align consecutive declarations in columns]"
    "--align-assignments[Align the operators of consecutive assignments]"
    "--align-comments[Align the trailing comments of consecutive lines]"
    "--sort-imports[Sort consecutive imports]"
    "--sort-modifiers[Sort the modifiers of a declaration]"
    "--scala3-syntax[Scala: convert the conditions to the Scala 3 syntax]"
    "--trailing-commas[Insert or remove the trailing comma of a list spanning lines]: :(always never)"
    "--scala3-end-markers[Scala: insert end markers after definitions of # lines or more]: :"
    "--pad-closure-braces[Scala: the spaces inside the braces of a one-line lambda]: :(none)"
    "--pad-block-braces[Scala: the spaces inside the braces of a one-line block]: :(none)"
    "--pad-import-braces[Scala: the spaces inside the braces of import selectors]: :(none)"
    "--pad-brace-call[Scala: the space before the brace of a method call]: :(none)"
    "--pad-pattern-at[Scala: the spaces around the @ of a pattern]: :(none)"
    "--pad-include[Insert space padding after include directives]: :(none)"
    "--pad-paren[Insert space padding around parenthesis on the outside, the inside, or both]: :(in out none)"
    ...
    "--pad-brackets[Insert space padding around square brackets on the outside, the inside, or both]: :(in out none)"
    "--pad-semicolon[Remove space padding before a semicolon]: :(none)"
    ...
    "--break-elseifs[Break 'else if()' statements into two different lines]: :(no-indent)"
    "--break-one-line-headers[Break one line headers (e.g]"
    "--line-between-members[Insert an empty line between class/struct method definitions and between top-level functions]: :(all)"
    ...
    "--remove-braces[Remove braces from braced conditional statements]"
    "--remove-braces=one-line[Remove braces from one-line conditional statements only (experimental)]"
    "--break-return-type[Break the return type from the function name definitions]: :(decl)"
    "--attach-return-type[Attach the return type to the function name definitions]: :(decl)"
    ...
    "--max-code-length-mode[Determines how line length is measured]: :(code total ignore-side-comments)"
    "--break-after-logical[After break line using --max-code-length, place logical conditional last on the previous line]"
    "--mode[Set input syntax mode]: :(c java cs objc js jsx ts tsx go rust kotlin swift dart scala gsc ghc)"
    "--pad-method-prefix[Insert space padding after the '-' or '+' Objective-C method prefix]: :(none)"
    "--pad-return-type[Insert space padding after the Objective-C return type]: :(none)"
    "--pad-param-type[Insert space padding after the Objective-C param type]: :(none)"
    ...
    "--error-on-changes[With --dry-run: Report any file reformat as error]"
    ...
    "--preserve-ws[Preserve whitespace near comma operators if squeeze-ws is not set]"
    "--indent-lambda[Indent C++ lambda functions (experimental, broken for complex fct bodies)]"
    "--block-continuation[Indent continuation lines one indent from the start of the statement]"
    "--align-continuation[Align continuation lines with an assignment or a paren]"
    "--accept-empty-list[With wildcard or -R\: Do not report an empty input file list as error]"
  )
  _arguments -s -S \
    $options \
    "*: :_files" \
